...
namespace QOA.DEMO.AccesoDatos
{
    public class PolizaDL : ConexionDL, IDisposable
    {
        ...
        public PolizaBE consultarDetalle(long idPoliza)
        {
            try
            {
                ...
                using (SqlConnection conex = new SqlConnection(conexion))
                {
                    ...
                    if (dr["B_AUTO_REEMPLAZO"] != DBNull.Value) p.autoReemplazo = Convert.ToInt32(dr["B_AUTO_REEMPLAZO"]) == 1;
                    if (dr["N_COSTO_AUTO_REEMPLAZO"] != DBNull.Value) p.costoAutoReemplazo = Convert.ToDouble(dr["N_COSTO_AUTO_REEMPLAZO"]);
        ...
        private void AgregarParametrosPoliza(SqlCommand cmd, PolizaBE p)
        {
            ...
            cmd.Parameters.Add("@B_AUTO_REEMPLAZO", SqlDbType.Bit).Value = p.tipoProducto == "VEHICULAR" && p.autoReemplazo; cmd.Parameters.Add("@N_COSTO_AUTO_REEMPLAZO", SqlDbType.Decimal).Value = p.tipoProducto == "VEHICULAR" ? p.costoAutoReemplazo : 0;
        }
